...
export const metadata = {
  title: 'تسجيل منظمة جديدة | Ezidi Events Worldwide',
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-300 text-xs font-semibold">
          ...
          <span>{isAr ? 'السجل الرسمي للمنظمات والمؤسسات الإيزيدية' : 'Official Organization Registry'}</span>
        </div>
        <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
          {isAr ? 'تسجيل منظمة أو مؤسسة جديدة' : 'Register Your Organization'}
        </h1>
        <p className="text-sm sm:text-base text-slate-400 max-w-2xl mx-auto">
          {isAr
            ? 'يمكن للمنظمات غير الربحية، المراكز الثقافية، والجمعيات الحقوقية الإيزيدية التسجيل للحصول على التوثيق الرسمي ونشر الفعاليات المعتمدة.'
      ...
      <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 text-xs sm:text-sm text-slate-300 flex items-start gap-3">
        ...
        <div className="space-y-1">
          <span className="font-bold text-white block">
            {isAr ? 'نظام التوثيق والتدقيق الأمني المحمي' : 'Private Verification Pipeline'}
          </span>
          <p className="text-slate-400 text-xs leading-relaxed">
            {isAr
              ? 'تُحفظ جميع المستندات والمعلومات بشكل مشفر وخاص، وتتم مراجعتها حصرياً من قبل المشرفين الإداريين للمنصة قبل اعتماد المنظمة.'
      ...
      <OrgRegisterClientForm countries={countries} cities={cities} locale={locale} />
    </div>
  );
}
